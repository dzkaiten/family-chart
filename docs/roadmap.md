...
- [x] Off-site daily backup repo (`family-tree-backups`, private) — cron export of `tree_data.data` to committed JSON + owner-gated restore script

---

## In progress / open

- [ ] **Encrypt backup data at rest** in `family-tree-backups` — the private repo
      currently commits plaintext family data. Evaluate encrypting the JSON (and whether
      to recreate the repo to drop plaintext history). *(see spec.md §10)*
