...
export function cardPrimaryName(data: Record<string, unknown>, lang: LanguageCode = currentLanguage): string {
  ...
  // Primary name follows the UI language: Chinese mode shows the Chinese name,
  // English mode shows the English name, each falling back to the other.
  if (isChinese(lang)) return cn || en;
  return en || cn;
}
